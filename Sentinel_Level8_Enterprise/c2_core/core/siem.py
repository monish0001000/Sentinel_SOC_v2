...
class LogRepository:
    ...
    async def log_event(self, level, message, source, log_type, metadata=None):
        try:
            ...
            # Publish to EventBus for real-time WebSocket streaming
            log_entry = {
                "id": log_id,
                "timestamp": timestamp,
                "level": level,
            ...
            await self.bus.publish("siem_event", log_entry)
    ...
    def search_logs(self, query_text: str, limit: int = 200):
        """Full-text search across message, source, type, and metadata."""
        cursor = self.conn.cursor()
        like_pattern = f"%{query_text}%"
        sql = """
            SELECT * FROM logs
            WHERE message LIKE ? OR source LIKE ? OR type LIKE ? OR metadata LIKE ?
            ORDER BY timestamp DESC LIMIT ?
        """
        cursor.execute(sql, (like_pattern, like_pattern, like_pattern, like_pattern, limit))
        rows = cursor.fetchall()
        return [dict(row) for row in rows]
